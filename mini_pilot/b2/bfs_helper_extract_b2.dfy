...
class BreadthFirstSearch<Vertex(==)>
{
    ...
    ensures d < 0 ==> !exists p :: IsPath(source, dest, p)
  {
      ...
      decreases AllVertices - Processed
    {
      ...
      var newlyEncountered := NewlyEncounteredBroken(v, V);
      assert if C == {} then
  ...
  ghost method NewlyEncounteredBroken(v: Vertex, V: set<Vertex>) returns (s: set<Vertex>)
    ensures s == set w | w in Succ(v) && w !in V
  {
    s := set w | w in Succ(v) && w !in V;
  }
}
