  ...
  ensures forall j, k :: 0 <= j < k < N ==> B[j] != B[k] // (which means that) B is injective
{
  ...
  EstablishInverseBroken(N, A, B);
}

ghost function inImage(i: int): bool { true }  // this function is used to trigger the surjective quantification

lemma EstablishInverseBroken(N: int, A: array<int>, B: array<int>)
  requires 0 <= N && N == A.Length && N == B.Length
  requires forall k :: 0 <= k < N ==> 0 <= A[k] < N
  requires forall k :: 0 <= k < N ==> B[A[k]] == k
  requires forall m :: 0 <= m < N && inImage(m) ==> exists k :: 0 <= k < N && A[k] == m
  ensures forall j :: 0 <= j < N && inImage(j) ==> 0 <= B[j] < N
{
}
