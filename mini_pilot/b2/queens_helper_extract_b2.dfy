...
method MakeCandidateBroken(boardSoFar: seq<int>, n: nat) returns (candidate: seq<int>)
  ensures candidate == boardSoFar + [n]
{
  candidate := boardSoFar + [n];
}
  ...
  decreases N - |boardSoFar|
{
      ...
      exists p :: 0 <= p < N && !IsConsistent(B, p)
  {
    ...
    var candidateBoard := MakeCandidateBroken(boardSoFar, n);
