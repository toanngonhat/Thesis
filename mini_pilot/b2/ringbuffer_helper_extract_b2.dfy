...
class RingBuffer<T(0)>
{
  ...
  {
    var nextEmpty := NextEmptyBroken(start, len, data.Length);
    data[nextEmpty] := x;
    len := len + 1;
    Contents := Contents + [x];
  }
...
method NextEmptyBroken(start: nat, len: nat, dataLength: nat) returns (idx: int)
  ensures 0 <= idx < dataLength
  ensures idx == if start + len < dataLength then start + len else start + len - dataLength
{
  idx := if start + len < dataLength then start + len else start + len - dataLength;
}
