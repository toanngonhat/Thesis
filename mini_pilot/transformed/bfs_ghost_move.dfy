...
class BreadthFirstSearch<Vertex(==)>
{
    ...
    ensures d < 0 ==> !exists p :: IsPath(source, dest, p)
  {
      ...
      decreases AllVertices - Processed
    {
      ...
      ghost var pathToV := SnapshotPath(source, v, paths);
  ...
  ghost method SnapshotPath(source: Vertex, v: Vertex, paths: map<Vertex, List<Vertex>>)
      returns (pathToV: List<Vertex>)
    requires ValidMap(source, paths)
    requires v in paths
    ensures pathToV == Find(source, v, paths)
  {
    pathToV := Find(source, v, paths);
  }
}
