...
class BreadthFirstSearch<Vertex(==)>
{
    ...
    ensures d < 0 ==> !exists p :: IsPath(source, dest, p)
  {
      ...
      decreases AllVertices - Processed
    {
      ...
      var freshFrontier := set w | w in Succ(v) && w !in V;
      assert if C == {} then
        Processed + (N + freshFrontier) == R(source, d+1, AllVertices)
        else Processed + C == R(source, d, AllVertices) by { reveal R(); }
      V, N := V + freshFrontier, N + freshFrontier;
      paths := UpdatePaths(freshFrontier, source, paths, v, pathToV);
