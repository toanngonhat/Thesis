...
lemma PreserveLeftConsistency(boardSoFar: seq<int>, candidateBoard: seq<int>, pos: nat)
  requires candidateBoard == boardSoFar + [candidateBoard[pos]]
  requires pos == |boardSoFar|
  requires pos < |candidateBoard|
  requires IsConsistent(candidateBoard, pos)
  requires forall k :: 0 <= k < |boardSoFar| ==> IsConsistent(boardSoFar, k)
  ensures forall k :: 0 <= k < |boardSoFar| ==> IsConsistent(candidateBoard, k)
{
}
  ...
  decreases N - |boardSoFar|
{
      ...
      exists p :: 0 <= p < N && !IsConsistent(B, p)
  {
    ...
    if IsConsistent(candidateBoard, pos) {
      ...
      PreserveLeftConsistency(boardSoFar, candidateBoard, pos);
